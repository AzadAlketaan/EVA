...
@section('content')
    ...
    @if(!Auth::guest())
        @if(Gate::check('checkpermission','edit') )
            <a href="/reservation/{{$reservation->id}}/edit" class="btn btn-default">Edit</a>
        @endif
        <a href="/order/create" class="btn btn-default">Add order to reservation</a>
        @if( Gate::check('checkpermission' , 'destroy'))
            {!!Form::open(['action' => ['App\Http\Controllers\ReservationController@destroy', $reservation->id], 'method' => 'POST', 'class' => 'pull-right'])!!}
