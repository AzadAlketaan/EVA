...
@section('content')
    <h1>Edit Drink</h1>
    {!! Form::open(['action' => ['App\Http\Controllers\DrinkController@update', $drink->id], 'method' => 'POST', 'enctype' => 'multipart/form-data']) !!}
    <div class="form-group">
        ...
        @foreach($drinktype as $drinktypes)
            @if($drinktypes->id ==$drink->drinktype_id)  
                <option value='{{ $drink->drinktype_id }}' >{{ $drinktypes->Type_Name }}</option>
            @endif
        @endforeach
        @foreach($drinktype as $drinktypes)
            @if($drinktypes->id != $drink->drinktype_id)  
